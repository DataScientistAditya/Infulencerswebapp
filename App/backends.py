...
class AccountAuth(ModelBackend):
    ...
    def Get_Urls(Recipient, Token):
        ...
        Message = f"Your Account Needs to Be Verified http://adsbyinfluence.com/Verify/{Token}"
    ...
    def Reset_Pass(Recipient, email):
        Subject = "MightyNeurons Email Verification"
        Message = f"Reset Your Password Here, http://adsbyinfluence.com/Reset/{email}"
        Email_Sender = settings.EMAIL_HOST_USER
        Email_Reciever = [Recipient]
        send_mail(subject=Subject, message=Message, from_email=Email_Sender, recipient_list=Email_Reciever)
    ...
    def Check_Phone_Number(Number):
        try:
            if Number == "+41524204242" or Number != None:
    ...
    def Get_Usa_Taxpayer(userid):
        try:
            User_Country = Influencer_Details.objects.all().filter(User = userid).values("Country")[0]["Country"]
            if User_Country == "United States":
                return "Yes"
            else:
                return None
    ...
    def Filter_Fun(Demo_List):
        List_Tags = []
        for i in range(len(Demo_List)):
            Values = Demo_List[i].startswith("#")
            if Values == True:
                List_Tags.append(Demo_List[i])
        return List_Tags
